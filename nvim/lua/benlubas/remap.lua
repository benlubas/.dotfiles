...
nnoremap('<leader>e', '<cmd>w<CR><cmd>Ex<CR>') -- write quit to netrw
...
nnoremap('<leader>fj', '<cmd>Telescope current_buffer_fuzzy_find<CR>') -- Find in current file
nnoremap('<leader>fk', '<cmd>Telescope live_grep<CR>') -- Find a word in a file in the directory
...
nnoremap('<leader>fh', function() tb.find_files({ hidden = true }) end) -- include hidden files in directory search
nnoremap('<leader>fs', function() tb.symbols({ sources = { 'emoji', 'nerd', 'julia' } }) end) -- Symbols with extra sources
nnoremap('<leader>h', '<cmd>Telescope help_tags<CR>') -- search help tags
nnoremap('<leader>wh', function() tb.help_tags({ default_text = vim.fn.expand("<cword>") }) end) -- help with word under cursor
nnoremap('<leader>Wh', function() tb.help_tags({ default_text = vim.fn.expand("<cWORD>") }) end) -- help with WORD under cursor
vnoremap('<leader>h', function() tb.help_tags({ default_text = vim.fn.expand("<cword>") }) end) -- help with visual selection
...
nnoremap('<leader>tr', function() require('neotest').run.run() end) -- Run the nearest test
...
nnoremap('<leader>th', function() nt.consumer.output({ enter = true }) end) -- open output window
